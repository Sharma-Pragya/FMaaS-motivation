# gpu_run_model_inference_img_cls.py
import os
import re
import csv
import json
...
def load_dataset(json_path):
    import os, json
    from config import dataset  # absolute .../dataset/image_classification
    ...
    for rec in data:
        ...
        if not os.path.isabs(p):
            rec["image_path"] = os.path.join(dataset, p)  # joins .../image_classification + images/00000.jpg
    return data
...
# ---------------------- Image Classification prompt/parse ----------------------
PROMPT_TEXT = (
    "What is this image? "
...
def parse_label(text: str) -> str:
    """Parse the model's answer to extract a single class label."""
    if not text:
        return ""
    # Strip and lowercase
...
def run_inference(model, processor, data, handle):
    ...
    for i, item in enumerate(data):
        # Skip image 73 for Qwen model in img_cls task
        if is_qwen and i == 73:
        ...
        gt_label = item["label"].lower()  # Ground truth label (strict matching)
        ...
        # Check if prediction matches any variant in the ground truth label
        # GT labels can be comma-separated (e.g., "ringlet, ringlet butterfly")
        is_correct = (pred_label in gt_label)
        if is_correct:
            correct += 1
