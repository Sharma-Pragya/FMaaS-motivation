# gpu_run_model_inference_scene.py
import os
import re
import csv
import json
...
def load_dataset(json_path):
    import os, json
    from config import dataset  # absolute .../dataset/scene_classification
    ...
    for rec in data:
        ...
        if not os.path.isabs(p):
            rec["image_path"] = os.path.join(dataset, p)  # joins .../scene_classification + images/00000.jpg
    return data
...
# ---------------------- Scene Classification prompt/parse ----------------------
PROMPT_TEXT = (
...
def parse_label(text: str) -> str:
    """Parse the model's answer to extract a scene classification label."""
    if not text:
        return ""
    # Strip and lowercase
