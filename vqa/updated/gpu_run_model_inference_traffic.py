# gpu_run_model_inference_traffic.py
import os
import re
import csv
import json
...
def load_dataset(json_path):
    import os, json
    from config import dataset  # absolute .../dataset/traffic_classification
    ...
    for rec in data:
        ...
        if not os.path.isabs(p):
            rec["image_path"] = os.path.join(dataset, p)  # joins .../traffic_classification + images/00000.jpg
    return data
...
# ---------------------- Traffic Sign Classification prompt/parse ----------------------
PROMPT_TEXT = (
    "What traffic sign is shown in this image? "
    "Answer with the traffic sign name only, without any explanation. "
    "Use simple terms like 'stop', 'yield', 'speed limit 30', etc."
)

def parse_label(text: str) -> str:
    """Parse the model's answer to extract a traffic sign label."""
    if not text:
        return ""
    # Strip and lowercase
