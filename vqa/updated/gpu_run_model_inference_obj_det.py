# gpu_run_model_inference_obj_det.py
import os
import re
import csv
import json
...
from config import model_name as cfg_model_name, dataset, dataset_json, models_directory
...
def parse_args():
    ...
    p.add_argument("--log_file", type=str, default="metrics_log_linux.csv")
    return p.parse_args()
...
def load_dataset(json_path):
    import os, json
    from config import dataset  # absolute .../dataset/object_detection
    ...
    for rec in data:
        ...
        if not os.path.isabs(p):
            rec["image_path"] = os.path.join(dataset, p)  # joins .../object_detection + images/00000.jpg
    return data
...
def parse_category(text: str) -> str:
    """Parse the model's answer to extract a single category name."""
    if not text:
        return ""
    # Strip and lowercase
...
def run_inference(model, processor, data, handle):
    ...
    for i, item in enumerate(data):
        ...
        gt_categories = item["categories"]  # List of category names
        ...
        evaluated += 1
        pred_category = parse_category(answer)
        is_correct = (pred_category in gt_categories)
        if is_correct:
            correct += 1
        print(f"[{i}] GT categories: {gt_categories}  →  Pred: {pred_category}  |  Raw: {answer}")
