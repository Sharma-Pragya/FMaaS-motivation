# gpu_run_model_inference_crowd.py
import os
import re
import csv
import json
...
def load_dataset(json_path):
    import os, json
    from config import dataset  # absolute .../dataset/crowd_counting
    ...
    for rec in data:
        ...
        if not os.path.isabs(p):
            rec["image_path"] = os.path.join(dataset, p)  # joins .../crowd_counting + images/00000.jpg
    return data
...
# ---------------------- Categorical Crowd Counting prompt/parse ----------------------
PROMPT_TEXT = (
...
def parse_label(text: str) -> str:
    """Parse the model's answer to extract a crowd density category."""
    if not text:
        return ""
    # Strip and lowercase
