# gpu_run_model_inference_gesture.py
import os
import re
import csv
import json
...
def load_dataset(json_path):
    import os, json
    from config import dataset  # absolute .../dataset/gesture_recognition
    ...
    for rec in data:
        ...
        if not os.path.isabs(p):
            rec["image_path"] = os.path.join(dataset, p)  # joins .../gesture_recognition + images/00000.jpg
    return data
...
# ---------------------- Hand Gesture Recognition prompt/parse ----------------------
PROMPT_TEXT = (
...
def parse_label(text: str) -> str:
    """Parse the model's answer to extract a hand gesture label."""
    if not text:
        return ""
    # Strip and lowercase
