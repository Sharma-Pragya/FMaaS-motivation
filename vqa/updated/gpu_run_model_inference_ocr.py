# gpu_run_model_inference_ocr_mnist.py
import os
import re
import csv
import json
...
def load_dataset(json_path):
    import os, json
    from config import dataset  # absolute .../dataset/ocr
    ...
    for rec in data:
        ...
        if not os.path.isabs(p):
            rec["image_path"] = os.path.join(dataset, p)  # joins .../ocr + images/00000.png
    return data
...
def run_inference(model, processor, data, handle):
    ...
    for i, item in enumerate(data):
        ...
        evaluated += 1
        pred = parse_digit(answer) or ""
        is_correct = (pred == gt_digit)
        if is_correct:
            correct += 1
        print(f"[{i}] GT digit: {gt_digit}  →  Pred: {pred}  |  Raw: {answer}")
