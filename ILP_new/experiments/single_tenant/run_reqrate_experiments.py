"""
Request rate scaling experiments for single-tenant ILP.

Varies request rate (demand) while keeping task count fixed.

Usage:
    python run_reqrate_experiments.py                         # Run all experiments
    python run_reqrate_experiments.py --workload vlm          # Run only VLM
    python run_reqrate_experiments.py --rates 2 4 6           # Run specific rates
    python run_reqrate_experiments.py --tasks 6               # Use 6 tasks instead of 4

Results saved to: ./results/reqrate/
"""
...
# Fixed device pool for ALL experiments (same as scaling experiments)
CANONICAL_DEVICES = {
...
# Default task requirements (latency SLO)
DEFAULT_VLM_LATENCY_SLO = 1500  # ms
DEFAULT_TSFM_LATENCY_SLO = 100  # ms
...
def build_ilp_inputs_vlm(task_list, req_rate, components, pipelines, latency_data, metric_data):
    """Build ILP inputs for VLM workload with specified tasks and request rate."""
    ...
    # All tasks get the same request rate
    task_slos = {t: DEFAULT_VLM_LATENCY_SLO for t in task_list}
    demands = {t: float(req_rate) for t in task_list}
...
def build_ilp_inputs_tsfm(task_list, req_rate, components, pipelines, latency_data, metric_data):
    """Build ILP inputs for TSFM workload with specified tasks and request rate."""
    devices = {k: {"type": v["type"]} for k, v in CANONICAL_DEVICES.items()}
    vram_device = {"A6000": 48000.0, "A16": 16000.0}

    task_slos = {t: DEFAULT_TSFM_LATENCY_SLO for t in task_list}
    demands = {t: float(req_rate) for t in task_list}
    Amin = {t: 0.0 for t in task_list}
...
def build_ilp_inputs_mixed(vlm_tasks, tsfm_tasks, req_rate, components, pipelines, latency_data, metric_data):
    """Build ILP inputs for mixed workload with specified tasks and request rate."""
    devices = {k: {"type": v["type"]} for k, v in CANONICAL_DEVICES.items()}
    vram_device = {"A6000": 48000.0, "A16": 16000.0}
    device_types = set(info["type"] for info in devices.values())
    ...
    Amin = {}
    for t in vlm_tasks:
        task_slos[t] = DEFAULT_VLM_LATENCY_SLO
        demands[t] = float(req_rate)
        Amin[t] = 0.0
    for t in tsfm_tasks:
        task_slos[t] = DEFAULT_TSFM_LATENCY_SLO
        demands[t] = float(req_rate)
        Amin[t] = 0.0
...
def collect_metrics(result, ilp_inputs):
    ...
    # Total deployed capacity: sum of deployment throughputs
    # Each deployment serves exactly ONE task in single-tenant, so use min capacity (bottleneck)
    ...
    # For each deployment, find which task is actually routed to it
    deployment_throughput = {}
    for (m, d) in deployed:
        # Find tasks ACTUALLY routed to this deployment
        routed_tasks = [
            t for t in tasks
...
def run_single_experiment(workload_type, num_tasks, req_rate, objective_mode,
    ...
    if workload_type == "vlm":
        ...
        ilp_inputs = build_ilp_inputs_vlm(task_list, req_rate, components, pipelines, latency_data, metric_data)
    elif workload_type == "tsfm":
        task_list = select_tasks_roundrobin(TSFM_TASKS, num_tasks)
        ilp_inputs = build_ilp_inputs_tsfm(task_list, req_rate, components, pipelines, latency_data, metric_data)
        ...
        ilp_inputs = build_ilp_inputs_mixed(vlm_task_list, tsfm_task_list, req_rate,
                                            components, pipelines, latency_data, metric_data)
    ...
    print(f"  Request rate per task: {req_rate} req/s")
...
def main():
    parser = argparse.ArgumentParser(description="Request rate scaling experiments for single-tenant ILP")
    parser.add_argument(
        "--workload",
        type=str,
        nargs="+",
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[2, 4, 6, 8, 10],
        help="Request rates to test (req/s per task)",
    )
    parser.add_argument(
        "--tasks",
        type=int,
        default=4,
        help="Number of tasks (fixed)",
    )
    parser.add_argument(
        "--objective",
        type=str,
        nargs="+",
        default=["deployments_devices", "deployments_devices_modelsize"],
    ...
    total_runs = len(args.workload) * len(args.rates) * len(args.objective)
    print(f"Request Rate Scaling Experiment Plan:")
    print(f"  Workloads: {args.workload}")
    print(f"  Request rates: {args.rates} req/s per task")
    print(f"  Fixed tasks: {args.tasks}")
    ...
    results_base = os.path.join(os.path.dirname(__file__), "results", "reqrate")
    ...
    for workload_type in args.workload:
        ...
        for req_rate in args.rates:
            for objective_mode in args.objective:
                run_num += 1
                print(f"\n[{run_num}/{total_runs}] {workload_type} | {req_rate} req/s | {objective_mode}")

                exp_result = run_single_experiment(
                    workload_type, args.tasks, req_rate, objective_mode,
                    components, pipelines, latency_data, metric_data
                )
                ...
                result_file = os.path.join(
                    ...
                    f"{workload_type}_{req_rate}reqrate_{objective_mode}.json"
                )
                with open(result_file, 'w') as f:
                    json.dump(exp_result, f, indent=2)

                all_results[workload_type][objective_mode][req_rate] = exp_result["metrics"]
    ...
    for workload_type in args.workload:
        summary = {
            ...
            "req_rates": sorted(args.rates),
            "objective_modes": args.objective,
            "results": all_results[workload_type],
        }

        summary_file = os.path.join(results_base, "summary", f"{workload_type}_reqrate_summary.json")
