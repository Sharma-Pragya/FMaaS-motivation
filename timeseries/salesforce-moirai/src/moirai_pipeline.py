...
def get_morai_predictor(
    ...
    start = perf_counter()
    load_memory_before = get_gpu_memory_and_util(self.handle)["gpu_mem_used_mb"]
    ...
    load_memory= get_gpu_memory_and_util(self.handle)["gpu_mem_used_mb"]-self.load_memory_before
...
def run_moirai(
    ...
    init_time = time()
    predictor,load_duration,load_memory = get_morai_predictor(model_size, horizon, target_dim, batch_size)
