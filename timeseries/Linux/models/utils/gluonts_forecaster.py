...
from .log_collector import init_nvml, get_gpu_memory_and_util, get_cpu_memory_and_util
...
class GluonTSForecaster(Forecaster):
    ...
    def forecast(self, df: pd.DataFrame, h: int, freq: str):
        ...
        inference_data_loader = InferenceDataLoader(
            ...
            transform=predictor.input_transform + SelectFields(
                predictor.input_names + predictor.required_fields,
                allow_missing=True,
            ),
        ...
        with torch.no_grad():
            for batch in tqdm(inference_data_loader):
                ...
                # Generate forecasts for this batch only
                pred = list(forecast_generator(
                    inference_data_loader=[batch],  # Wrapping one batch in a list
                    prediction_net=predictor.prediction_net,
                    input_names=predictor.input_names,
                    output_transform=predictor.output_transform,
                    num_samples=100  # or predictor.num_samples if defined
                ))
