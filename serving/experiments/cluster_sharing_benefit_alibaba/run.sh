#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────
#  cluster_sharing_benefit_alibaba sweep: vary #apps and measure cluster latency.
#
...
#  Usage:
#    cd serving
#    bash experiments/cluster_sharing_benefit_alibaba/run.sh
#
#  Env overrides:
#    N_APPS_LIST   "8 16 24 32"    (defaults to user_config.n_apps_list)
#    CONDITIONS    "sharing no_sharing no_sharing_tpc"
#    DURATION      180
#    RESULTS_DIR   experiments/cluster_sharing_benefit_alibaba/results
...
DURATION="${DURATION:-}"
RESULTS_DIR="${RESULTS_DIR:-experiments/cluster_sharing_benefit_alibaba/results_alibaba}"
SKIP_GEN="${SKIP_GEN:-0}"
...
from experiments.cluster_sharing_benefit_alibaba.user_config import n_apps_list
print(' '.join(str(n) for n in n_apps_list))
")
fi
if [[ -z "$CONDITIONS" ]]; then
    CONDITIONS=$(python3 -c "
from experiments.cluster_sharing_benefit_alibaba.user_config import conditions
print(' '.join(conditions))
")
fi
if [[ -z "$DURATION" ]]; then
    DURATION=$(python3 -c "
from experiments.cluster_sharing_benefit_alibaba.user_config import experiment
...
if [[ "$SKIP_GEN" != "1" ]]; then
    ...
    python -m experiments.cluster_sharing_benefit_alibaba.deployments.generate
fi
...
run_one() {
    ...
    python -u -m experiments.cluster_sharing_benefit_alibaba.run \
        --n-apps     "$n" \
        --condition  "$cond" \
        --duration   "$DURATION" \
        --output-dir "$out_dir" \
