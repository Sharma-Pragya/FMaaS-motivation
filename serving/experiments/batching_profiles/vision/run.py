"""
Vision batching profile: DINOv2 backbone + nyudepth/vocseg, sweep batch sizes.

Usage (from serving/):
  python experiments/batching_profiles/vision/run.py [--backbone dinobase-patch] \
      [--task nyudepth] [--batch_sizes 1,2,4,8,16,32] \
...
TASK_CONFIG = {
    "nyudepth": {
        "decoder_type": "monocular",
        "decoder_path": "nyudepth_{backbone}_monocular",
    },
    "vocseg": {
...
def build_loader(task: str, batch_size: int):
    if task == "nyudepth":
...
def parse_args():
    ...
    p.add_argument("--backbone",    type=str, default="dinobase-patch")
    p.add_argument("--task",        type=str, default="nyudepth",
                   choices=list(TASK_CONFIG.keys()))
    p.add_argument("--batch_sizes", type=str, default="1,2,4,8,16,32")
...
def run_batch_size(runtime, task, batch_size, n_requests):
    loader    = build_loader(task, batch_size)
...
def main():
    ...
    decoder_path = cfg["decoder_path"].format(backbone=bb_short) if cfg["decoder_path"] else None
    ...
    print(f"[vision] Loading {backbone} + {task} decoder on {args.device}...")
    ...
    decoder_key = f"add_decoder_{decoder_path}" if decoder_path else f"add_decoder_{task}"
    ...
    with open(req_path, "a", newline="") as rf, \
        ...
        for bs in batch_sizes:
            print(f"[vision] batch_size={bs}  ({args.n_requests} requests)...")
            lats, bbs, decs, gpus = run_batch_size(runtime, task, bs, args.n_requests)
            ...
            sw.writerow([
                ...
                _s(decoder_key,       "wall time"),
                _s(decoder_key,       "gpu peak"),
                ...
                round(max(gpus), 3),        # peak_gpu_mem_mb
                round(np.mean(gpus), 3),    # avg_gpu_mem_mb
                round(throughput, 3),
            ...
            print(f"[vision]   p50={np.percentile(lat_arr,50):.1f}ms  "
                  f"p99={np.percentile(lat_arr,99):.1f}ms  "
                  f"throughput={throughput:.1f} samples/s")

    print(f"[vision] Results saved to {out_dir}")
