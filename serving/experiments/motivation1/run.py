...
Usage:
    python run.py [--n-tasks 10,8,6,4,2,1] [--duration 60] [--concurrency 4]
                  [--strategies task_sharing,deploy_sharing]
                  [--backbone chronosbase] [--cuda cuda:0]
                  [--max-batch-size 32] [--max-batch-wait-ms 10]
...
def run_benchmark(
    ...
    mem_log_path = str(result_dir / "memory_log.csv")
    server.start_memory_logging(mem_log_path, interval_s=0.5)
    ...
    # Compute avg allocated memory from the log file
    avg_gpu_mem_mb = 0.0
    try:
        with open(mem_log_path, newline="") as mf:
            rows_mem = list(csv.DictReader(mf))
        if rows_mem:
            avg_gpu_mem_mb = sum(float(r["allocated_mb"]) for r in rows_mem) / len(rows_mem)
    except Exception:
        pass
...
def main() -> int:
    ...
    parser.add_argument("--max-batch-size",       type=int,   default=int(os.environ.get("MAX_BATCH_SIZE", "32")))
    parser.add_argument("--max-batch-wait-ms",    type=float, default=float(os.environ.get("MAX_BATCH_WAIT_MS", "10")))
    ...
    print(f"  Max batch wait    : {args.max_batch_wait_ms} ms")
