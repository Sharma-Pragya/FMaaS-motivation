...
class _LocalVLLMEndpoint:
    ...
    def stop(self) -> None:
        if self.runtime is not None:
            try:
                # shut down vLLM's async background loop so it releases tensor references
                engine = getattr(self.runtime, "model", None)
                async_engine = getattr(engine, "_async_engine", None)
...
class LLMInferenceServer:
    ...
    def stop(self) -> None:
        # Stop the event loop first so no async callbacks hold tensor references
