#!/usr/bin/env bash
# noisy_neighbor/vision — Vision-backbone interference experiment
#
# Loops over scheduler policies (sharing) plus no-sharing baselines, running each
# with fresh device server(s). Heavy vision kernels (dinolarge ~60–120ms) make
# kernel-level contention visible in process-level no_sharing.
# Run from serving/:
#   bash experiments/noisy_neighbor/vision/run.sh
#
...
# Vision device server needs to find finetuned decoders
export DECODER_DIR="${DECODER_DIR:-${FMTK_DIR}/models/vision/finetuned}"

DEVICE_PORT="${DEVICE_PORT:-8000}"
CUDA_DEVICE="${CUDA_DEVICE:-cuda:0}"
BACKBONE="${BACKBONE:-dinolarge-patch}"
VICTIM_TASK="${VICTIM_TASK:-vocseg}"
AGGRESSOR_TASK="${AGGRESSOR_TASK:-nyudepth}"
...
RESULTS_BASE="${RESULTS_BASE:-experiments/noisy_neighbor/vision/results}"
...
echo "================================================================"
echo "  noisy_neighbor/vision — ${NUM_PHASES}-phase experiment"
echo "  Backbone     : $BACKBONE"
echo "  Victim       : $VICTIM_TASK @ ${VICTIM_RPS} rps (constant)"
echo "  Aggressor    : $AGGRESSOR_TASK"
for (( i=0; i<NUM_PHASES; i++ )); do
