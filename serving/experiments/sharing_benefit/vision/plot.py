#!/usr/bin/env python3
"""sharing_benefit/vision/plot.py — Plots for the vision sharing-benefit experiment.

Produces:
  1. sharing_benefit_latency_cdf.png      — CDF of per-request latency, one line per condition
  2. sharing_benefit_throughput_cdf.png   — CDF of completion throughput (req/s, 1s bins)
  3. sharing_benefit_summary_bars.png     — bar chart: p99 latency per condition per task

Usage:
    python experiments/sharing_benefit/vision/plot.py [--exp-dir experiments/sharing_benefit/vision/results]
"""
...
CONDITION_ORDER = ["single_nyudepth", "single_vocseg", "no_sharing", "sharing"]
...
def load_series_latencies(
    ...
    single_nyudepth + single_vocseg are averaged into one 'single' series.
    """
    ...
    single_lats = []
    for cond in ("single_nyudepth", "single_vocseg"):
...
def load_series_throughput(
    ...
    def _bin_throughput(completions: List[float]) -> List[float]:
        if not completions:
            return []
        # Use wall-clock aligned 1s bins so the counts are interpretable as
        # "completed requests in each second" rather than bins starting at an
        # arbitrary completion timestamp offset.
        t_min = float(np.floor(min(completions)))
        t_max = float(np.ceil(max(completions)))
    ...
    single_completions = (
        _completion_times(result_root, "single_nyudepth") +
        _completion_times(result_root, "single_vocseg")
    )
    if single_completions:
        counts = _bin_throughput(single_completions)
        series["single"] = [c / 2.0 for c in counts]

    for cond in ("no_sharing", "sharing"):
        completions = _completion_times(result_root, cond)
        if completions:
            counts = _bin_throughput(completions)
            series[cond] = [c / 2.0 for c in counts]
...
def _observed_batch_stats(rows: List[Dict]) -> Dict:
    """Estimate observed batch sizes from server_start_ns.

    Requests that share the same server_start_ns are in the same batch.
    """
...
def load_batch_sizes(
    ...
    """Returns {series: avg_batch_size} for one RPS level."""
    raw: Dict[str, float] = {}
    for cond in CONDITION_ORDER:
        lat_file = result_root / cond / "latencies.csv"
        if not lat_file.exists():
        ...
        if rows:
            stats = _observed_batch_stats(rows)
            raw[cond] = stats["mean"]

    series: Dict[str, float] = {}
    single_vals = [raw[c] for c in ("single_nyudepth", "single_vocseg") if c in raw]
    if single_vals:
...
# ---------------------------------------------------------------------------
# Plot 5: Average batch size vs RPS (grouped bars)
# ---------------------------------------------------------------------------

def plot_batch_size_sweep(
    all_batch: Dict[int, Dict[str, float]],
    rps_list: List[int],
    out_path: Path,
) -> None:
    """Grouped bar chart: x = RPS level, bars = condition, y = avg batch size."""
    rps_with_data = [r for r in rps_list if r in all_batch]
    if not rps_with_data:
        print("[Warn] No batch size data, skipping plot")
        return

    n_rps = len(rps_with_data)
    n_series = len(SERIES_ORDER)
    bar_width = 0.22
    x = np.arange(n_rps)

    fig, ax = plt.subplots(figsize=(max(3.3, 0.9 * n_rps), 1.6))

    for i, s in enumerate(SERIES_ORDER):
        vals = [all_batch.get(rps, {}).get(s, 0.0) for rps in rps_with_data]
        offset = (i - (n_series - 1) / 2) * bar_width
        bars = ax.bar(
            x + offset, vals, bar_width,
            color=SERIES_COLORS[s], edgecolor="black", linewidth=0.4,
            label=SERIES_LABELS[s], zorder=2,
        )
        for bar in bars:
            h = bar.get_height()
            if h > 0:
                ax.text(bar.get_x() + bar.get_width() / 2, h + 0.05,
                        f"{h:.1f}", ha="center", va="bottom", fontsize=4.5)

    ax.set_xticks(x)
    ax.set_xticklabels([str(r) for r in rps_with_data])
    ax.set_xlabel("Request Rate (req/s per task)")
    ax.set_ylabel("Avg Batch Size")
    ax.set_ylim(0)
    ax.grid(axis="y", zorder=0)
    ax.set_axisbelow(True)
    ax.legend(handles=_legend_handles(), frameon=False, loc="upper left",
              ncol=1, handlelength=1.5, handletextpad=0.3, fontsize=5.5)
    fig.tight_layout()
    save_figure(fig, out_path)
    plt.close(fig)
...
def main() -> int:
    ...
    parser.add_argument("--rps-sweep",       default="5,8,10,15,20,25,30,35,40,45,50,60,80,100,120",
    ...
    rps_with_batch = [r for r in rps_list if r in all_batch]
    if rps_with_batch:
        plot_batch_size_sweep(all_batch, rps_with_batch,
                              out_dir / "sharing_benefit_avg_batch_size.pdf")
