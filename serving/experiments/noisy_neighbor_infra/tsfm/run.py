...
async def run_timeseries(
    ...
    Each task gets its own EdgeRuntimeClient pointed at task_urls[task], so
    victim and aggressor can target different device servers (non-sharing runs).
...
def main() -> int:
    ...
    # Deploy each task's decoder to its own server.
    # When both URLs are the same (sharing / single-server), the second deploy
    # is a no-op because the backbone is already loaded; both decoders are
    # included in the first call to avoid a race on the same server.
