...
AGGRESSOR_RPS_PHASES="${AGGRESSOR_RPS_PHASES:-5,100,10}"
...
# Format: "run_name"  (always uses fifo scheduler, batch_size=1, batch_wait=0)
NO_SHARING_TPC_RUNS=("no_sharing_tpc")
TPC_MODE="${TPC_MODE:-libsmctrl}"

RESULTS_BASE="${RESULTS_BASE:-experiments/noisy_neighbor_infra/tsfm/results}"
...
stop_devices() {
    ...
    DEVICE_PIDS=()
    ACTIVE_PORTS=()
    sleep 2   # give OS time to release GPU memory / ports
}
trap 'stop_devices' EXIT

# start_shared_device SCHEDULER BATCH_SIZE BATCH_WAIT LOG
#   Starts a single device server for both tasks (sharing condition).
start_shared_device() {
...
# start_tpc_device PORT TASK TPC_PARTITION LOG
#   Starts a TPC-pinned device server for a single task.
#   Prints the PID on stdout; all other output goes to stderr.
start_tpc_device() {
    local port="$1" task="$2" tpc_partition="$3" log="$4"
    local task_rates="${task}:${VICTIM_RPS}"
    pkill -f "device/main.py.*--port ${port}" 2>/dev/null || true
    sleep 1
    echo "[run.sh] Starting TPC device: port=$port task=$task tpcs=[$tpc_partition]" >&2
    $PYTHON -u "$SERVING_DIR/device/main.py" \
        --port              "$port"          \
        --runtime-type      pytorch          \
        --cuda              "$CUDA_DEVICE"   \
...
for run in "${RUNS[@]}"; do
    ...
    $PYTHON -u experiments/noisy_neighbor_infra/tsfm/run.py \
...
# ---------------------------------------------------------------------------
for RUN_NAME in "${NO_SHARING_TPC_RUNS[@]}"; do
    EXP_DIR="${RESULTS_BASE}/${RUN_NAME}"

    echo ""
    echo "================================================================"
    echo "  $RUN_NAME  (TPC-isolated, 2 servers, tpc_mode=$TPC_MODE)"
    echo "  Results: $EXP_DIR"
    echo "================================================================"
...
")
    ...
    # Split TPCs evenly between victim (first half) and aggressor (second half)
    HALF=$((TOTAL_TPCS / 2))
    ...
    start_tpc_device "$VICTIM_PORT"    "$VICTIM_TASK"    "$VICTIM_TPCS"    "$LOG_DIR/device_${RUN_NAME}_victim.log"
    start_tpc_device "$AGGRESSOR_PORT" "$AGGRESSOR_TASK" "$AGGRESSOR_TPCS" "$LOG_DIR/device_${RUN_NAME}_aggressor.log"
