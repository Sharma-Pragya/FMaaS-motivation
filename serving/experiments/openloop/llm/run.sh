#!/usr/bin/env bash
# motivation2/llm — Open-loop LLM overhead experiment
#
...
#
# Run from serving/:
#   bash experiments/motivation2/llm/run.sh
...
RPS_SWEEP="${RPS_SWEEP:-2,5,10,20,30,40,50}"
DURATION="${DURATION:-30}"
MAX_NEW_TOKENS="${MAX_NEW_TOKENS:-64}"
MAX_MODEL_LEN="${MAX_MODEL_LEN:-256}"
GPU_UTIL="${GPU_UTIL:-0.85}"
EXP_DIR="${EXP_DIR:-experiments/motivation2/llm/results}"
DEVICE_STARTUP_WAIT="${DEVICE_STARTUP_WAIT:-15}"   # vLLM needs time to start
PYTHON="${PYTHON:-/home/hshastri_umass_edu/.conda/envs/fmtk_vllm/bin/python}"

LOG_DIR="experiments/motivation2/llm/logs"
mkdir -p "$LOG_DIR"
...
echo "================================================================"
echo "  motivation2/llm"
echo "  Backbone    : $BACKBONE"
echo "  N-tasks     : ${TASK_LIST[*]}"
echo "  RPS sweep   : ${RPS_LIST[*]}"
echo "  Duration    : ${DURATION}s per point"
echo "  max_new_tok : $MAX_NEW_TOKENS"
echo "  Total points: $(( ${#TASK_LIST[@]} * ${#RPS_LIST[@]} ))"
echo "================================================================"
...
start_device() {
    ...
    pkill -f "motivation2/llm/run.py" 2>/dev/null || true
...
for n_tasks in "${TASK_LIST[@]}"; do
    for rps in "${RPS_LIST[@]}"; do
        ...
        "$PYTHON" -u experiments/motivation2/llm/run.py \
