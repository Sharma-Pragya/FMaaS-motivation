#!/usr/bin/env bash
# fair_share/vision — Three-phase priority experiment with phased victim load
#
...
#
# Run from serving/:
#   bash experiments/fair_share/vision/run.sh
...
RESULTS_BASE="${RESULTS_BASE:-experiments/fair_share/vision/results_t4}"
...
echo "================================================================"
echo "  fair_share/vision — saturation experiment"
echo "  Backbone     : $BACKBONE"
echo "  Client A (priority/victim): $VICTIM_TASK"
echo "  Client B (other/aggressor): $AGGRESSOR_TASK"
IFS=',' read -ra _VRPS <<< "$VICTIM_RPS_PHASES"
...
start_shared_device() {
    ...
    pkill -f "fair_share/vision/run.py" 2>/dev/null || true
    sleep 1
...
for run in "${RUNS[@]}"; do
    ...
    $PYTHON -u experiments/fair_share/vision/run.py \
