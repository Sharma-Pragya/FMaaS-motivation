#!/usr/bin/env python3
"""fair_share/vision — Vision-backbone priority experiment.

Shows phased victim/aggressor load across N phases on vision tasks
(nyudepth + vocseg). Records per-request latency timeseries for plotting.
"""
...
def main() -> int:
    ...
    parser.add_argument("--backbone",             default="dinobase-patch")
    parser.add_argument("--victim-task",          default="nyudepth")
    parser.add_argument("--aggressor-task",       default="vocseg")
    ...
    parser.add_argument("--exp-dir",              default=os.environ.get(
                        "EXP_DIR", "experiments/fair_share/vision/results_t4/stfq_1_1"))
    ...
    print("=" * 65)
    print(f"  fair_share/vision — {num_phases}-phase experiment")
    ...
    bb_short = args.backbone.replace("-patch", "")

    def _decoder_entry(task: str) -> Dict:
        tmpl = DECODER_PATHS.get(task)
        path = tmpl.format(backbone=bb_short) if tmpl else None
        return {"task": task, "type": TASK_TYPES[task], "path": path}

    if victim_url == aggressor_url:
        decoders = [_decoder_entry(t) for t in tasks]
