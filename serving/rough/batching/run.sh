#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────
#  run.sh — End-to-end experiment runner
#
...
#  Usage:
#    cd serving
#    bash experiments/baselines/run.sh
#
#  To customise:
#    SCHEDULERS="fmaas clipper"  RATES="10 50"  bash experiments/baselines/run.sh
# ─────────────────────────────────────────────────────────────────────
set -euo pipefail

# ── Configurable parameters (override via environment) ───────────────
SCHEDULERS="${SCHEDULERS:- fmaas fmaas_share}"
RATES="${RATES:-100 150 200}"
TRACE="${TRACE:-lmsyschat}"
DURATION="${DURATION:-360}"
SEED="${SEED:-42}"
EXP_DIR="${EXP_DIR:-experiments/batching/results}"
EXP_TYPE="${EXP_TYPE:-batching}"
PLOT_DIR="${PLOT_DIR:-experiments/batching/plots}"
...
for sched in $SCHEDULERS; do
    for rate in $RATES; do
        ...
        COMMON_ARGS="--scheduler $sched --req-rate $rate --duration $DURATION --trace $TRACE --seed $SEED --exp-dir $EXP_DIR --exp-type $EXP_TYPE"
