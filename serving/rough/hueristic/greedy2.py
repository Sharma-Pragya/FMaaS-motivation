...
def deploy_task(task, share_flag=False, do_fit=True):
    ...
    print("Sorted backbones to try:", sorted_backbones)
    #print accuracy metric for sorted backbones
    for pid, backbone in enumerate(sorted_backbones):
        for p_id, pipeline in pipelines.items():
            if pipeline['task']==task[0] and pipeline['backbone']==backbone:
                print("Backbone:", backbone, "Metric:", get_pipeline_task_accuracy(p_id))
                break
    for b in sorted_backbones:
        print("Trying backbone:", b)
        m, demand_left = deploy_model(task, b, share_flag)
        print("Demand left after trying backbone:", demand_left)
        if demand_left <=1e-9:
            return m, demand_left
    if share_flag:
        active_backbones_endpoints=[]
...
if __name__ == "__main__":
    from hueristic.user_config import devices, tasks
    deployments=shared_packing(devices,tasks)
    final_json = build_final_json(deployments)
    with open("hueristic/deployment_plan.json", "w") as f:
